...
name: gh-export
description: >-
  Formats all confirmed pentest findings from `pentest/findings.json` into copy-pasteable GitHub issue markdown blocks, following the AppSec reporting guide template. Reads the snapshot regenerated by `pentester/refresh.py`, groups findings by severity, and emits one issue block per finding with title, severity label, reproduction steps, raw evidence, remediation (if present), and CWE/OWASP/CVE references.

  Use when the user asks to file pentest findings as GitHub issues, export a pentest report into an issue tracker, generate copy-paste-ready issue templates from `findings.json`, or hand off scan output to a remediation workflow.
